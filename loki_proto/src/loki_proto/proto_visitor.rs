...
impl Protovisitor {
    pub fn get_structure_from_msg_type(&self, _msg_type: String) ->  Result<&ProtoMessage> {
        ...
        return Err(anyhow!("cannot find the message with _msg_type {}", _msg_type));
    }
...
impl<'input> Loki_protoVisitor<'input> for Protovisitor {
    ...
    fn visit_message(&mut self, ctx: &MessageContext<'input>) {
        ...
        let ctx_msg_name = ctx.messageName().expect("messageName parsed unsuccessfully");
        if self.prefix == "" {
        ...
        let ctx_msg_body = ctx.messageBody().expect("messageBody parsed unsuccessfully");
        ...
        for ctx_field in ctx_field_all{
            ...
            let ctx_field_name = ctx_field.fieldName().expect("fieldName parsed unsuccessfully");
            attr_tmp.set_attr_name(ctx_field_name.get_text());
            // print!("\t attrName is {}", ctx_field_name.get_text());

            let ctx_field_number = ctx_field.fieldNumber().expect("fieldNumber parsed unsuccessfully");
        ...
        for ctx_oneof in ctx_oneof_all{
            ...
            let ctx_oneof_name = ctx_oneof.oneofName().expect("oneofName parsed unsuccessfully");
            ...
            for ctx_oneof_field in ctx_oneof_field_all{
                let ctx_ele_type = ctx_oneof_field.eleType().expect("eletype parsed unsuccessfully");
                s+=&ctx_ele_type.get_text();

                let ctx_field_name = ctx_oneof_field.fieldName().expect("fieldName parsed unsuccessfully");
                s+=" ";
                s+=&ctx_field_name.get_text();

                let ctx_field_number = ctx_oneof_field.fieldNumber().expect("fieldNumber parsed unsuccessfully");
                s+=" ";
        ...
        for ctx_map in ctx_mapfield_all{
            ...
            let ctx_field_number = ctx_map.fieldNumber().expect("keyType parsed unsuccessfully");
            attr_tmp.set_attr_value(ctx_field_number.get_text());
    ...
    fn visit_enumDefinition(&mut self, ctx: &EnumDefinitionContext<'input>) { 
        ...
        for ctx_enum_field in ctx_enum_field_all{
            ...
            let attr_name = ctx_enum_field.Ident().expect("enumField Ident parsed unsuccessfully");
            // println!("enum field name is : {}", attr_name.get_text());

            let attr_value = ctx_enum_field.IntLit().expect("enumField Ident parsed unsuccessfully");
